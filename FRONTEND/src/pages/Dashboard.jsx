...
function Cat() {
  ...
  return (
    <div
      style={{ position: "absolute", left: 100, bottom: 350, width: 260, pointerEvents: "auto" }}
      onMouseEnter={() => play()}
      onMouseLeave={() => stop()}
    >
      {View}
...
function PortraitFrame({ user, onEdit, onEditName }) {
  ...
  return (
    ...
    >
      ...
      <svg width="138" height="158" viewBox="0 0 138 158" style={{ display: "block" }}>
        ...
        {isEditing ? (
          <foreignObject x="34" y="135" width="70" height="16">
            <input
              autoFocus
              value={tempName}
      ...
      {/* Overlays (for tooltips/pencil icon) moved outside SVG to avoid invalid structure */}
      <div style={{ position: "absolute", top: 138, left: 101, pointerEvents: "auto" }}>
        <Tip text="Edit display name" pos="bottom">
          <div onClick={handleStartEdit} style={{ width: 12, height: 12, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", opacity: 0.85 }}>
            <svg width="10" height="10" viewBox="0 0 10 10">
              <path d="M0 7 L2 9 L7 4 L5 2 Z M6 1 L8 3" stroke="#F5DEB3" strokeWidth="1.2" fill="none" />
            </svg>
          </div>
        </Tip>
      </div>

      <div style={{ position: "absolute", top: 12, left: 106, pointerEvents: "auto" }}>
        <Tip text="Account Settings" pos="right">
          <div style={{ width: 14, height: 14, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", opacity: 0.85 }}>
            <svg width="12" height="12" viewBox="0 0 12 12">
              <circle cx="6" cy="6" r="2.5" fill="none" stroke="#F5DEB3" strokeWidth="1.2" />
              {[0, 45, 90, 135, 180, 225, 270, 315].map(a => (
                <line key={a} x1="6" y1="6" x2={6 + 5.5 * Math.cos(a * Math.PI / 180)} y2={6 + 5.5 * Math.sin(a * Math.PI / 180)}
                  stroke="#F5DEB3" strokeWidth="1.5" strokeLinecap="round" />
              ))}
            </svg>
          </div>
        </Tip>
...
function WallClock({ session }) {
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{ position: "absolute", top: 48, left: 610, pointerEvents: "auto", cursor: "pointer", zIndex: 10 }}>
...
function StreakCalendar({ streakDays }) {
  ...
  return (
    <Tip text="Your reading calendar" pos="left" style={{ position: "absolute", top: 236, left: 1050, pointerEvents: "auto" }}>
      <div style={{
        ...
        <div style={{
          ...
          <button onClick={() => setDisplayDate(new Date(year, month - 1, 1))}
            style={{
              background: "none", border: "none", color: "rgba(255,240,220,.7)",
...
function EmotionLamp({ genre }) {
  ...
  return (
    <Tip text={`Mood: ${genre.charAt(0).toUpperCase() + genre.slice(1)}`} pos="right">
      <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
        style={{
          position: "absolute", top: 310, left: 60, pointerEvents: "auto",
...
function Shelves({ onNav }) {
  ...
  return (
    <>
      ...
      <div style={{ position: "absolute", left: 800, top: 38, display: "flex", alignItems: "flex-end", gap: 3, height: 98, pointerEvents: "auto" }}>
        {books.map((b, i) => (
          <NavBook key={b.title} {...b} tilt={i === 3 ? 8 : 0} onClick={() => onNav(b.title)} />
        ))}
        {/* Two leaning filler books for realism */}
        <div style={{
          width: 17, height: 63, flexShrink: 0,
          background: "linear-gradient(to right,#5A3018,#7A5030)",
          borderRadius: "2px 3px 3px 2px",
          transform: "rotate(16deg) translateY(5px)", opacity: "1"
        }} />
        <div style={{
          width: 13, height: 70, flexShrink: 0,
          background: "linear-gradient(to right,#253D25,#3A5A38)",
          borderRadius: "2px 3px 3px 2px",
          transform: "rotate(-7deg) translateY(3px)", opacity: "1"
        }} />
        <div style={{
          width: 9, height: 46, flexShrink: 0,
          background: "linear-gradient(to bottom,#C8903C,#8B6020)",
          borderRadius: 2, marginLeft: 4, opacity: "1",
          boxShadow: "2px 0 5px rgba(0,0,0,.3)"
        }} />
      </div>

      {/* ── SHELF 2: Trophies ── */}
      <div style={{ position: "absolute", left: 750, top: 168, display: "flex", alignItems: "flex-end", gap: 8, padding: "0 12px", height: 56, pointerEvents: "auto" }}>
...
function ArchiveBox({ onClick }) {
  ...
  return (
    <div onClick={onClick} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{
        position: "absolute", top: 490, left: 450, pointerEvents: "auto",
        cursor: "pointer",
...
function Dustbin({ deleted }) {
  return (
    <div style={{ position: "absolute", top: 518, left: 1200, pointerEvents: "auto", zIndex: 5 }}>
      <svg width="54" height="62" viewBox="0 0 54 62">
        ...
        {deleted.slice(0, 2).map((s, i) => (
          <foreignObject key={s.id} x={15 + i * 10} y={35 - i * 4} width="20" height="20">
            <Tip text={`🗑 "${s.title}"`} pos="left">
              <div style={{ width: 12 - i, height: 12 - i, borderRadius: "50%", background: i === 0 ? "#E0D090" : "#D8C880", opacity: 0.72, cursor: "default" }} />
            </Tip>
          </foreignObject>
        ))}
        {/* lid */}
...
function FloorDecor() {
  return (
    <>
      {/* Stacked books (left) */}
      <div style={{ position: "absolute", top: 528, left: 60, pointerEvents: "auto" }}>
      ...
      {/* School backpack */}
      <div style={{ position: "absolute", top: 506, left: 160, pointerEvents: "auto" }}>
      ...
      {/* Rug */}
      <div style={{ position: "absolute", top: 562, left: 260, width: 280, height: 18, pointerEvents: "none" }}>
      ...
      {/* Stuffed bear */}
      <div style={{ position: "absolute", top: 534, left: 560, pointerEvents: "auto" }}>
      ...
      <div style={{ position: "absolute", top: 548, left: 680, pointerEvents: "auto" }}>
      ...
      {/* Small toy blocks */}
      <div style={{ position: "absolute", top: 538, left: 880, pointerEvents: "auto" }}>
      ...
      {/* Toy train */}
      <div style={{ position: "absolute", top: 544, left: 980, pointerEvents: "auto" }}>
...
function DreamCatcher() {
  return (
    <div style={{
      position: "absolute", top: "1%", left: "8%", pointerEvents: "none", zIndex: 20,
  ...
  return (
    <>
      {[{ s: "left:1%", d: "0s" }, { s: "left:13%", d: "0.9s" }, { s: "right:1%", d: "1.5s" }].map((c, i) => (
        <div key={i} style={{
          position: "absolute", top: 0, [Object.keys(c)[0]]: Object.values(c)[0].split(":")[1],
          pointerEvents: "none", animationDelay: c.d, zIndex: 20,
...
function SceneOverlay({ children }) {
...
export default function LofiDashboard() {
  const [user, setUser] = useState(() => {
    const saved = localStorage.getItem("lofi_user");
  ...
  <style>
    ...
    `}
          ...
          {/* Pencil cup */}
          <rect x="920" y="402" width="36" height="38" rx="5" fill="#3A2410" stroke="#6A4A24" strokeWidth="1.2" />
          <line x1="931" y1="402" x2="929" y2="380" stroke="#FFB347" strokeWidth="2.8" strokeLinecap="round" />
          <line x1="938" y1="402" x2="937" y2="377" stroke="#8B4A2A" strokeWidth="2.5" strokeLinecap="round" />
          <line x1="945" y1="402" x2="946" y2="382" stroke="#C84030" strokeWidth="2.8" strokeLinecap="round" />
          <line x1="951" y1="402" x2="952" y2="379" stroke="#4A7A30" strokeWidth="2.5" strokeLinecap="round" />
          {/* Mug */}
          <rect x="970" y="408" width="32" height="32" rx="5" fill="#6A4028" stroke="#8B5830" strokeWidth="1.2" />
          <path d="M1002 416 Q1015 424 1002 434" fill="none" stroke="#8B5830" strokeWidth="2" />
          <ellipse cx="986" cy="408" rx="16" ry="4.5" fill="#7A5030" />
          {/* Small boxes on desk */}
